...
public class MakePads {
	...
	private Map<Integer, Map<Integer, Integer>> slidePad = new HashMap<Integer, Map<Integer, Integer>>();
		...
		for (int l = 0; l <= 9; l++) {
			for (int c = 0; c <= 9; c++) {
				...
				if (Arrays.asList(btnsIDs).contains(l + " " + c)) {
					if ((l == 0 || l == 9) && (c == 0 || c == 9)) {
						cantov = new RelativeLayout(context); //context.getLayoutInflater().inflate(R.layout.cantos, null);
						//cantov.setLayoutParams();
						if (l == 0 && c == 9) {
							cantov = (RelativeLayout) context.getLayoutInflater().inflate(R.layout.pad, null);
							...
							//cantov = new RelativeLayout(context);//(RelativeLayout) context.getLayoutInflater().inflate(R.layout.cantos, null);
