...
public class Readers {
		...
		if (folders == null || folders.length == 0) {
			...
			for (final File projectFolder : folders) {
				...
				if (info.exists()) {
					...
					infoInfo.put(ProjectsAdapter.TITLE,
							title.replaceFirst(title.substring(0, title.indexOf("=") + 1), "").trim());
					infoInfo.put(ProjectsAdapter.PRODUCER_NAME, producerName
					...
					infoInfo.put(ProjectsAdapter.PRODUCER_NAME, context.getString(R.string.incomplet_project));
	...
	private static boolean checkkeyLED(Activity context, String line, String fileName, int led_code) {
		...
		case "o":
			 ...
			 */
			boolean accept_mapper = line.matches(
			...
			if (accept_mapper) { // verificar o codigo de cor
				...
				if (line.substring(line.indexOf("a") + 2).matches("([0-9]|[1-9][0-9]|1([0-1][0-9]|2[0-7]))")) {
					return true;
	...
	protected static Map<String /*chain+pad position+repeat*/, List<List<String>> /*led file readed*/> readKeyLEDs(
			Activity context, File keyLED_Path, int start_index, int end_index) {
				...
				try {
					...
					for (String line : keys) {
						if (!line.replace(" ", "").isEmpty()) {
							...
							//	if (line.contains("*"))
							String orLine = line;
							line = line.trim().replace("*", "mc").replace("off", "f").replace("on", "o")
									.replace("auto", "a").replace("delay", "d").replace("logo", "l");
					...
					if ((!line.replaceAll("\\s", "").isEmpty()))
						if (checkKeySound(line.replaceAll("\\s", ""))) {
							...
							try {
								...
								PlayPads.mSoundLoader.loadSound(sound, sound_length, line.substring(0, indextheSound),
										ifToChain);
							} catch (IllegalArgumentException i) {
								PlayPads.invalid_formats.add("(" + keySound.getName() + ")"
										+ context.getString(R.string.invalid_sound) + " " + line);
							}
						} else {
							PlayPads.invalid_formats.add("(" + keySound.getName() + ")"
									+ context.getString(R.string.invalid_sound) + " " + line);
		...
		try {
			...
			while (line != null) {
				if (!line.replaceAll(" ", "").isEmpty()) {
					line = line.replaceAll("chain", "c").replaceAll(" ", "").replaceAll("delay", "d").replaceAll("off",
							"f");
	...
	public static void getColorTableForCTFile(File rootDir, int list_pos, boolean EYEDROP) {
		...
		File[] files_list_index = rootDir.listFiles(new FileFilter() {
			...
			public boolean accept(File pathname) {
				return (pathname.isFile()
						&& pathname.getName().substring(pathname.getName().indexOf(".")).equals(FILE_EXTENSION));
			}
		});
		if (EYEDROP) {
		...
		try {
			BufferedReader ct_file = new BufferedReader(
					new FileReader(rootDir.getPath().toString() + "/" + files_list_index[list_pos].getName()));
	...
	public static ArrayAdapter listColorTable(Context context) {
		final String FILE_EXTENSION = ".ct";
		String[] file_ct_name = new File(VariaveisStaticas.COLOR_TABLE_PATH).list(new FilenameFilter() {
			@Override
